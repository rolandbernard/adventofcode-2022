...
fn main() {
    ...
    dist.insert((0, (-1, start)), 0);
    queue.push(Reverse((0, 0, (-1, start))));
    while let Some(Reverse((_, t, (y, x)))) = queue.pop() {
        if y == height {
            println!("Result: {:?}", t);
            break;
        } else {
            ...
            for (dx, dy) in [(0, 1), (0, -1), (1, 0), (-1, 0), (0, 0)] {
                ...
                if nx >= 0
                    ...
                    && !bliz.contains(&(ny, nx))
                {
                    let key = ((t + 1) % period, (ny, nx));
                    let old_dist = dist.get(&key).cloned().unwrap_or(usize::MAX);
                    if t + 1 < old_dist {
                        dist.insert(key, t + 1);
                        queue.push(Reverse((
                            t as i64 + 1 + (height - ny).abs() + (width - nx).abs(),
                            t + 1,
                            (ny, nx),
                        )));
                    }
